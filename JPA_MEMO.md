...
```
모여 있는 것을 비워낸다,
즉 영속성 컨텍스트에 쌓여있는 데이터는 엔티티매니저가 자체적으로 디비에 영속화를 해주지만
사실 개발자가 의도한 타이밍에 디비 영속화가 일어나지 않는다.
개발자가 원하는 타이밍에 디비에 영속화를 시켜주기위해 .flush() 메소드를 사용한다.
영속성 캐시에 쌓여서 아직 반영되지 않은 엔티티의 변경을 flush 메소드 사용시점에 모두 반영해주는 역할을 한다.

그렇다고 flush를 남발하면 오히려 영속성 캐시의 장점을 모두 무효화 시키니 적당히 잘 사용하는 것이 중요하다. (flush를 통해 예상치 못한 동작들이 발생한다.)
```
...
```
context
    프레임워크에서 컨테이너들이 관리하고 있는 내용물들
    스프링의 빈들을 로딩하고 관리하는 등 일련의 작업들은 스프링 컨텍스트 위에서 활용 됨
    
    persistence 영속화
        사라지지 않고 지속적으로 접근 할 수 있다.
            > 
...
### Entity LifeCycle (생애주기)
```
    엔티티매니저가 관리하는 엔티티가 어떤식으로 처리 되는지.

    영속성 컨텍스트에서 가장 주체가 되는 클래스는 EntityManger 객체.
    
    엔티티는 하나의 객체이기 때문에 엔티티와 디비레코드 사이에 어떻게 연결해주고 어떻게 디비에 반영 해주는지 -> 모두 엔티티매니저의 역할

    영속성 컨텍스트 내에서 엔티티매니저가 엔티티의 상태를 어떻게 변화 시키는지.

    엔티티의 라이프사이클이라 하면, 4가지의 상태가 존재한다.
        1. 비영속 상태 (new, Transient)
            영속성 컨텍스트가 해당 엔티티 객체를 관리하지 않는 상태.
                엔티티 어노테이션 중 @Transient
        2. 영속 상태 ()
        3. 준영속 상태 (디테치?)
        4. 삭제 상태 (리무브)
    
    영속성 컨텍스트 관리하에 엔티티는 변경감지(더티체크), 조회에 대해서 1차 캐시를 제공하고,
    쓰기지연 등과 같은 이러한 것들의 처리에 대해서 개발자가 신경쓰지 않아도 되는 일종의 편의 성을 제공 해준다.
    그러나 신경쓰지 않았던 부분에서 개발자가 의도하지 않았던 동작이 발생하기도 한다.
